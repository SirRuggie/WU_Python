...
| Native upload modals are unavailable in the installed SDK | `.venv/bin/python`: Hikari reports `2.6.0`; `hikari.impl` exposes no names containing `Upload` or `Label` | Use a slash-command attachment without forcing an unrelated framework upgrade. |
