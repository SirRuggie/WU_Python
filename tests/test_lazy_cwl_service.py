...
def test_reminder_message_splits_large_roster_and_whitelists_mentions(monkeypatch):
    ...
    asyncio.run(service._send_reminder_message(doc, away))
...
def test_stale_reminder_and_finish_do_not_change_replacement(monkeypatch):
    old = _list_doc("old", clan_tag="ABC")
    old["status"] = "finished"
    replacement = _list_doc("new", clan_tag="ABC")
    mongo = _fake_mongo([old, replacement])
    scheduler = FakeScheduler()
    _wire(monkeypatch, mongo=mongo, scheduler=scheduler, links={})

    reminders = asyncio.run(service.set_reminders(
        "#ABC", True, every_minutes=30, expected_list_id="old",
    ))
    finished = asyncio.run(service.finish("#ABC", expected_list_id="old"))

    assert reminders == {"ok": False, "stale": True, "error": "This saved list changed. Refresh and try again."}
    assert finished == {"ok": False, "stale": True, "error": "This saved list changed. Refresh and try again."}
    current = asyncio.run(store.get_active(mongo, "#ABC"))
    assert current["_id"] == "new"
    assert current["reminders"]["enabled"] is False
