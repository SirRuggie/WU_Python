...
def _matches(document, query):
    for key, expected in query.items():
        ...
        if "$ne" in expected:
            ...
            is_forbidden = forbidden in actual if isinstance(actual, list) else actual == forbidden
            if is_forbidden:
                return False
    return True
...
class _Mongo:
    def __init__(self, documents=(), legacy_documents=()):
        ...
        self.lazy_cwl_snapshots = _Collection(legacy_documents)
...
def test_ensure_indexes_creates_named_indexes():
    mongo = _Mongo()
    ...
    legacy_keys, legacy_kwargs = mongo.lazy_cwl_lists.index_calls[3]
    assert legacy_keys == "legacy_snapshot_id"
    assert legacy_kwargs["unique"] is True
    assert legacy_kwargs["sparse"] is True
...
def test_expected_list_id_prevents_replacement_mutations():
    old = _list_doc("old", clan_tag="ABC")
    replacement = _list_doc("new", clan_tag="ABC")
    # Simulate a completed old list followed by a new saved list.
    old["status"] = "finished"
    mongo = _Mongo([old, replacement])

    with pytest.raises(lazy_cwl_store.StaleListError):
        asyncio.run(lazy_cwl_store.add_player(
            mongo, "ABC", {"tag": "#P1", "name": "One", "town_hall": 15},
            expected_list_id="old", now=NOW,
        ))
    with pytest.raises(lazy_cwl_store.StaleListError):
        asyncio.run(lazy_cwl_store.remove_players(
            mongo, "ABC", ["#P1"], expected_list_id="old",
        ))
    assert asyncio.run(lazy_cwl_store.set_reminders(
        mongo, "ABC", enabled=True, every_minutes=30, expected_list_id="old",
    )) is None
    assert asyncio.run(lazy_cwl_store.finish(
        mongo, "ABC", expected_list_id="old",
    )) is None
    assert mongo.lazy_cwl_lists.documents["new"]["players"] == []
    assert mongo.lazy_cwl_lists.documents["new"]["reminders"]["enabled"] is False
    assert mongo.lazy_cwl_lists.documents["new"]["status"] == "active"


def test_add_player_write_filter_rejects_a_player_added_after_the_precheck(monkeypatch):
    mongo = _Mongo([_list_doc("list-1", clan_tag="ABC")])
    original = mongo.lazy_cwl_lists.find_one_and_update

    async def raced(query, update, **kwargs):
        # Model a second request completing after this request's pre-check but
        # before its atomic write filter is evaluated.
        mongo.lazy_cwl_lists.documents["list-1"]["players"].append({
            "tag": "#P1", "name": "Other request", "town_hall": 15,
        })
        return await original(query, update, **kwargs)

    monkeypatch.setattr(mongo.lazy_cwl_lists, "find_one_and_update", raced)
    with pytest.raises(lazy_cwl_store.PlayerAlreadyListedError):
        asyncio.run(lazy_cwl_store.add_player(
            mongo, "ABC", {"tag": "#P1", "name": "One", "town_hall": 15}, now=NOW,
        ))
    assert len(mongo.lazy_cwl_lists.documents["list-1"]["players"]) == 1
...
def test_migrate_legacy_active_snapshot_preserves_reminder_and_is_idempotent():
    legacy = {
        ...
        "players": [{"tag": "#p1", "name": "One", "th_level": 15, "discord_id": "123"}],
    }
    mongo = _Mongo(legacy_documents=[legacy])
    asyncio.run(lazy_cwl_store.ensure_indexes(mongo))

    assert asyncio.run(lazy_cwl_store.migrate_legacy_active_snapshots(mongo, now=NOW)) == 1
    created = asyncio.run(lazy_cwl_store.get_active(mongo, "#ABC"))
    assert created["legacy_snapshot_id"] == "legacy-1"
    assert created["players"][0]["town_hall"] == 15
    assert created["players"][0]["discord_id"] == 123
    assert created["reminders"]["enabled"] is True
    assert created["reminders"]["sent_count"] == 4
    assert created["expires_at"] == datetime(2026, 9, 16, tzinfo=timezone.utc)
    assert mongo.lazy_cwl_snapshots.documents["legacy-1"]["active"] is False
...
def test_migration_keeps_legacy_row_when_an_unrelated_active_list_conflicts():
    legacy = {
        "_id": "legacy-1", "clan_tag": "#ABC", "clan_name": "Legacy",
        "snapshot_date": NOW, "active": True, "players": [],
    }
    mongo = _Mongo([_list_doc("dashboard-list", clan_tag="ABC")], [legacy])
    asyncio.run(lazy_cwl_store.ensure_indexes(mongo))

    assert asyncio.run(lazy_cwl_store.migrate_legacy_active_snapshots(mongo, now=NOW)) == 0
    assert mongo.lazy_cwl_snapshots.documents["legacy-1"]["active"] is True
    assert mongo.lazy_cwl_lists.documents["dashboard-list"]["status"] == "active"
