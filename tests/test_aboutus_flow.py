...
from extensions.commands.setup import recruit_aboutus as join_family
from extensions.components import registered_functions


GUILD_ID = join_family.ABOUT_US_GUILD_ID
USER_ID = 1234
...
def test_aboutus_next_step_and_target_are_updated():
    parts = join_family.build_aboutus()
    text = content.text_nodes(parts)[-1].content
    assert "unlock WU Strike System" in text
    assert "https://" not in text
    assert join_family.ABOUT_US_ROLE_ID == 1553110276251979937
    assert join_family.STRIKE_SYSTEM_CHANNEL_ID == 1547242610819604560


def test_aboutus_action_is_persistent_and_does_not_edit_public_post():
    action = registered_functions["aboutus_acknowledge"]
    assert action.no_return is True
    assert action.preload_state is False


def test_aboutus_grants_the_configured_role_after_rest_guild_check():
    ctx = _ctx()
    bot = _bot()

    _run(ctx, bot)

    bot.rest.fetch_channel.assert_awaited_once_with(join_family.STRIKE_SYSTEM_CHANNEL_ID)
    bot.rest.add_role_to_member.assert_awaited_once_with(
        guild=GUILD_ID, user=USER_ID, role=join_family.ABOUT_US_ROLE_ID,
    )
    ...
    assert button.url == f"https://discord.com/channels/{GUILD_ID}/{join_family.STRIKE_SYSTEM_CHANNEL_ID}"
    assert button.label == "Continue to WU Strike System"


def test_aboutus_refuses_a_panel_used_from_another_guild_before_granting():
...
def test_aboutus_is_idempotent_for_existing_role():
    ctx = _ctx()
    bot = _bot(member_roles=(join_family.ABOUT_US_ROLE_ID,))
...
def test_aboutus_role_grant_failure_is_private_and_keeps_public_post_unchanged():
    ctx = _ctx()
    bot = _bot()
