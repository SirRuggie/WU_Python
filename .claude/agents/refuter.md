...
Repo verification is `python -m pytest -q` on touched tests plus the two greps in
`docs/editing-this-repo.md` on touched Python. Also attack: two copies of one rule that
can drift, and docs or tests left stale by the change.
