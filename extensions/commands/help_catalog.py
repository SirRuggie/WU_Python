...
HELP_CATEGORIES = {
    ...
    "tickets": {
        "name": "Tickets",
        "emoji": "🎫",
        "description": "Thread recruitment, console search, flags, and migration",
        "commands": [
            ("/ticket approve", "Approve the current ticket. Recruiter/Admin only."),
            ("/ticket deny", "Deny the current ticket. Recruiter/Admin only."),
            ("/ticket find", "Search permanent ticket history. Recruiter/Admin only."),
            ("/ticket history", "Open one member's permanent ticket history. Recruiter/Admin only."),
            ("/ticket flags", "View active applicant flags. Recruiter/Admin only."),
            ("/ticket flag-add", "Add or update an applicant flag. Recruiter/Admin only."),
            ("/ticket flag-remove", "Deactivate an applicant flag. Recruiter/Admin only."),
            ("/ticket setup", "Post the ticket entry panel. Admin only."),
            ("/ticket configure-threads", "Validate and save candidate/staff thread parents. Admin only."),
            ("/ticket thread-config", "Revalidate thread parents and recruiter roles. Admin only."),
            ("/ticket console", "Post, inspect, or repair the shared recruiter console. Admin only."),
            ("/ticket config", "Inspect thread ticket settings. Admin only."),
            ("/ticket migrate-store", "Copy legacy ticket rows to the tickets collection. Admin only."),
            ("/ticket migrate-legacy", "Preview or resume one terminal legacy ticket clone. Admin only."),
            ("/ticket approve-migration-pilot", "Continue after verifying the 1–5 ticket pilot. Admin only."),
        ],
    },
    "cwl": {
        "name": "CWL & Reminders",
