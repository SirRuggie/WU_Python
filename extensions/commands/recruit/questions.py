...
async def primary_questions(
    ...
    **kwargs
    ...
    elif choice in question_content.PRIMARY_VARIANTS:
...
async def th_select(
    ...
    **kwargs
    ...
    try:
        template = await question_content.load_template(mongo, int(ctx.guild_id), "fwa_bases_upon_approval")
        components = question_content.render_template(
            template, user_id=int(user.id), recruiter_id=int(ctx.member.id),
            stage="result", town_hall=friendly_name, th_number=th_number,
            base_info=base_info, base_link=base_link,
