...
from extensions.commands.tickets_legacy.manage import safe_text_content
...
class MigrateStore(
    ...
    async def invoke(
        ...
        # --- coexistence guard --------------------------------------------
        # The only legacy edit the thread ticket system requires. Once
        # `/tickets migrate-store` has activated the canonical `tickets`
        # dataset, this v2 transform would overwrite it and its unique
        # channel_id index would reject thread tickets. Refuse outright.
        setup_doc = await mongo.ticket_setup.find_one({"_id": "config"}) or {}
        if setup_doc.get("ticket_store_activation_version") is not None:
            body = "\n".join([
                *header,
                "",
                "🛑 **STOPPED — nothing was written.**",
                "",
                "The thread ticket system has already activated the canonical "
                "`tickets` dataset. This legacy migration would overwrite it. "
                "Use `/tickets migrate-store` instead.",
            ])
